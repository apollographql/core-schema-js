...
export function directive<
  ...
  N extends string,
  A extends Shape,
  On extends DirectiveLocationEnum
>(spec: S, name: N, args: A, repeat: 'on' | 'repeatable on', ...locations: On[]): Directive<S, N, A, On> {
  return {
    spec,
    name,
    args,
...
export type AnyDirective = Directive<any, any, any, any>

export type Directive_DataOf<D extends AnyDirective> =
  D extends AnyDirective
    ? SpecifiedData<
        Specified_SpecOf<D>,
        Specified_NameOf<D>,
        Directive_NodeOf<D>,
        Must<Shape_DeTypeOf<Directive_ArgsOf<D>>>
      >
    : never

export type Directive_ArgsOf<D extends AnyDirective> =
  D extends Directive<any, any, infer A, any>
    ? A
    : never

export type Directive_NodeOf<D extends AnyDirective> =
  D extends Directive<any, any, any, infer On>
    ? NodeFor<On>
    : never

export type NodeFor<L extends DirectiveLocationEnum>
  =
  L extends 'SCHEMA'
